...
std::shared_ptr<Variable> SteadyStateProblem::rho(int i) { return _variables.at("rho").at(i); };
std::shared_ptr<Variable> SteadyStateProblem::phi_p(int i) { return _variables.at("phi_pipe").at(i); };
std::shared_ptr<Variable> SteadyStateProblem::phi_c(int i) { return _variables.at("phi_compressor").at(i); };
std::shared_ptr<Variable> SteadyStateProblem::alpha(int i) { return _variables.at("alpha").at(i); };
std::shared_ptr<Variable> SteadyStateProblem::s(int i) { return _variables.at("s").at(i); };
std::shared_ptr<Variable> SteadyStateProblem::d(int i) { return _variables.at("d").at(i); };
std::shared_ptr<Variable> SteadyStateProblem::slack_production(int i) { return _variables.at("slack_production").at(i); };
std::shared_ptr<Constraint> SteadyStateProblem::nodal_balance(int i) { return _constraints.at("nodal_balance").at(i); };
std::shared_ptr<Constraint> SteadyStateProblem::pipe_physics(int i) { return _constraints.at("pipe_physics").at(i); };
std::shared_ptr<Constraint> SteadyStateProblem::compressor_physics(int i) { return _constraints.at("compressor_physics").at(i); };
...
void SteadyStateProblem::add_constraints(const InputParams & ip) {
    add_nodal_balance_constraints();
    add_pipe_physics_constraints();
    add_compressor_physics_constraints();
...
void SteadyStateProblem::add_phi_variables() {
    ...
    _variables["phi_pipe"] = std::unordered_map<int, std::shared_ptr<Variable>>();
    ...
    _variables["phi_compressor"] = std::unordered_map<int, std::shared_ptr<Variable>>();
...
void SteadyStateProblem::add_nodal_balance_constraints() {
    ...
    _constraints["nodal_balance"] = std::unordered_map<int, std::shared_ptr<Constraint>>();
    ...
    for (auto node_index : _ssd.get_slack_node_indexes()) {
        auto constraint = _model.add_constraint("nodal_balance_" + std::to_string(node_index));
        ...
        auto out_compressors_in_node = _ssd.get_out_compressor_indexes_in_node(node_index);
        auto in_compressors_in_node = _ssd.get_in_compressor_indexes_in_node(node_index);
        for (auto out_pipe_index : out_pipes_in_node)
            constraint->add_term(Term(phi_p(out_pipe_index), area_pipe.get_value(out_pipe_index)));
        for (auto out_compressor_index : out_compressors_in_node)
            constraint->add_term(Term(phi_c(out_compressor_index), area_compressor.get_value(out_compressor_index)));
        for (auto in_pipe_index : in_pipes_in_node)
            constraint->add_term(Term(phi_p(in_pipe_index), -area_pipe.get_value(in_pipe_index)));
        for (auto in_compressor_index : in_compressors_in_node)
            constraint->add_term(Term(phi_c(in_compressor_index), -area_compressor.get_value(in_compressor_index)));
...
void SteadyStateProblem::add_pipe_physics_constraints() {
    auto slack_nodes = _ssd.get_slack_node_indexes();
    auto resistance = _ssd.get_resistance_pipe();
    auto pslack = _ssd.get_pslack();
    _constraints["pipe_physics"] = std::unordered_map<int, std::shared_ptr<Constraint>>();
    _dual_values["pipe_physics"] = std::unordered_map<int, double>();
    /* pipe physics */
    for (auto pipe_index : _ssd.get_pipe_indexes()) {
        auto constraint = _model.add_constraint("pipe_physics_" + std::to_string(pipe_index));
        auto fnode_index = _ssd.get_fnode_index_of_pipe(pipe_index);
        auto tnode_index = _ssd.get_tnode_index_of_pipe(pipe_index);
        bool is_fnode_slack = slack_nodes.find(fnode_index) != slack_nodes.end();
        if (is_fnode_slack)
            constraint->add_term(Term(std::pow(pslack.get_value(fnode_index), 2)));
        else
            constraint->add_term(Term(rho(fnode_index), 1.0, TermType::quadratic));
        constraint->add_term(Term(rho(tnode_index), -1.0, TermType::quadratic));
        constraint->add_term(Term(phi_p(pipe_index), -resistance.get_value(pipe_index), TermType::x_abs_x));
        constraint->equal_to(0.0);
...
void SteadyStateProblem::add_compressor_physics_constraints() {
    auto slack_nodes = _ssd.get_slack_node_indexes();
    auto resistance = _ssd.get_resistance_compressor();
    auto pslack = _ssd.get_pslack();
    _constraints["compressor_physics"] = std::unordered_map<int, std::shared_ptr<Constraint>>();
    _dual_values["compressor_physics"] = std::unordered_map<int, double>();
    /* compressor physics */
    for (auto compressor_index : _ssd.get_compressor_indexes()) {
        auto constraint = _model.add_constraint("compressor_physics_" + std::to_string(compressor_index));
        ...
        bool is_fnode_slack = slack_nodes.find(fnode_index) != slack_nodes.end();
        if (is_fnode_slack)
            constraint->add_term(Term(alpha(compressor_index), std::pow(pslack.get_value(fnode_index), 2), TermType::quadratic));
        else
            constraint->add_term(Term({alpha(compressor_index), rho(fnode_index)}, 1.0, TermType::x_sq_y_sq));
        constraint->add_term(Term(rho(tnode_index), -1.0, TermType::quadratic));
        constraint->add_term(Term(phi_c(compressor_index), -resistance.get_value(compressor_index), TermType::x_abs_x));
        constraint->equal_to(0.0);
...
void SteadyStateProblem::add_compressor_discharge_pressure_bounds() {
    ...
    auto pslack = _ssd.get_pslack();
    auto p_max = _ssd.get_p_max();
    _constraints["compressor_discharge_bounds"] = std::unordered_map<int, std::shared_ptr<Constraint>>();
    _dual_values["compressor_discharge_bounds"] = std::unordered_map<int, double>();
    /* discharge pressure from a compressor bounds */
    for (auto compressor_index : _ssd.get_compressor_indexes()) {
        auto constraint = _model.add_constraint("compressor_discharge_bounds_" + std::to_string(compressor_index));
        auto fnode_index = _ssd.get_fnode_index_of_compressor(compressor_index);
        bool is_fnode_slack = slack_nodes.find(fnode_index) != slack_nodes.end();
        if (is_fnode_slack)
            constraint->add_term(Term(alpha(compressor_index), pslack.get_value(fnode_index)));
        else
            constraint->add_term(Term({ alpha(compressor_index), rho(fnode_index) }, 1.0, TermType::xy));
        constraint->two_sided(0.0, p_max.get_value(fnode_index));
        _constraints["compressor_discharge_bounds"][compressor_index] = constraint;
        _dual_values["compressor_discharge_bounds"][compressor_index] = std::nan("");
    }
};
...
void SteadyStateProblem::add_objective(const InputParams & ip) {
    ...
    /* objective term 1: sum (g in gnodes) (- d(g) cd(g) + s(g) cs(g)) + sum (i in slack) slack_production(i) cslack(i)*/
