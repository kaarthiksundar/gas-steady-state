...
Model::Model()
    : _name(""), _variables(), _constraints(), _objective(), _num_variables(0),
      _num_constraints(0), _has_objective(false), _primal_solution(),
      _dual_solution(), _objective_value(), _solver_status(),
      _hessian_sparsity_pattern(){};

Model::Model(std::string name)
    : _name(name), _variables(), _constraints(), _objective(),
      _num_variables(0), _num_constraints(0), _has_objective(false),
      _primal_solution(), _dual_solution(), _objective_value(),
      _solver_status(), _hessian_sparsity_pattern(){};
...
int Model::get_nnz_hessian_lag() {
    int nnz;
    auto variables_with_nz_second_derivative =
        _objective->get_variable_ids_with_nz_second_derivative();
    auto variable_pairs_with_nz_mixed_second_derivative =
        _objective->get_variable_id_pairs_with_nz_mixed_second_derivative();
    for (auto constraint : _constraints) {
        auto s1 = constraint->get_variable_ids_with_nz_second_derivative();
        auto s2 =
            constraint->get_variable_id_pairs_with_nz_mixed_second_derivative();
        variables_with_nz_second_derivative.insert(s1.begin(), s1.end());
        variable_pairs_with_nz_mixed_second_derivative.insert(s2.begin(),
                                                              s2.end());
    }
    nnz = variables_with_nz_second_derivative.size() +
          variable_pairs_with_nz_mixed_second_derivative.size();
    return nnz;
};

/* get_bounds_info functions */
std::tuple<double, double> Model::get_variable_bounds(int i) {
    return _variables[i]->get_bounds();
};
std::tuple<double, double> Model::get_constraint_bounds(int i) {
    return _constraints[i]->get_bounds();
};
...
/* eval_f functions */
double Model::evaluate_objective_function_value(const double *vals) {
    return _objective->get_value(vals);
};

/* eval_grad_f functions */
std::vector<std::tuple<int, double>>
Model::evaluate_objective_gradient(const double *vals) {
...
std::vector<std::tuple<int, double>>
Model::evaluate_constraint_gradient(int i, const double *vals) {
...
std::vector<std::pair<int, int>> Model::get_hessian_sparsity_pattern() {
    if (_hessian_sparsity_pattern.size() == 0) {
        std::set<std::pair<int, int>> hessian_sparsity_pattern =
            _objective->get_hessian_sparsity();
        for (auto i = 0; i < _num_constraints; ++i) {
            auto constraint_hessian_sparsity_pattern =
                _constraints[i]->get_hessian_sparsity();
            hessian_sparsity_pattern.insert(
                constraint_hessian_sparsity_pattern.begin(),
                constraint_hessian_sparsity_pattern.end());
        }
        std::vector<std::pair<int, int>> sparsity_pattern(
            hessian_sparsity_pattern.begin(), hessian_sparsity_pattern.end());
        _hessian_sparsity_pattern = sparsity_pattern;
    }
    return _hessian_sparsity_pattern;
};
std::vector<std::tuple<int, int, double>>
Model::evaluate_objective_hessian(const double *vals) {
    return _objective->get_hessian(vals);
};
std::vector<std::tuple<int, int, double>>
Model::evaluate_constraint_hessian(int i, const double *vals) {
...
void Model::set_objective_value(double objective_value) {
    _objective_value = objective_value;
};

/* final solution getters for non-ipopt classes */
double Model::get_primal_value(std::shared_ptr<Variable> var) {
    return _primal_solution[var->get_id()];
};
double Model::get_dual_value(std::shared_ptr<Constraint> constraint) {
    return _dual_solution[constraint->get_id()];
};
...
Model::add_variables(int n, std::vector<std::string> names) {
    if (names.size() != n) {
        std::cerr << "variable names count not matching number of variables"
                  << std::endl;
        std::exit(1);
    }
    std::vector<std::shared_ptr<Variable>> vars;
...
std::ostream &operator<<(std::ostream &os, const Model &model) {
    os << "model with name (" << model._name
       << ") is a minimization problem with " << std::endl;
    os << "objective function: " << std::endl;
    os << *(model._objective) << std::endl;
    os << "variables: " << std::endl;
    for (auto var : model._variables)
        os << *var;
    os << "constraints: " << std::endl;
    for (auto constraint : model._constraints)
        os << *constraint;
    return os;
};
