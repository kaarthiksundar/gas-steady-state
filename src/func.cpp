...
Func::get_gradient(int num_variables, const double *var_values) {
...
std::set<int> Func::get_variable_ids_with_nz_second_derivative() {
    return _variable_ids_with_nz_second_derivative;
};
std::set<std::pair<int, int>>
Func::get_variable_id_pairs_with_nz_mixed_second_derivative() {
...
Func::get_hessian(const double *var_values) {
    std::vector<std::pair<int, int>> indices;
    std::vector<double> hessian_values;
    for (auto term : _terms) {
        auto tt = term.get_term_type();
        if (tt == TermType::linear || tt == TermType::constant)
            continue;
        auto var_ids = term.get_variable_ids();
        std::vector<double> values;
        for (auto var_id : var_ids)
        ...
        if (std::abs(mixed_derivative) > 1e-12) {
            int var_id_x = var_ids[0], var_id_y = var_ids[1];
            auto var_id_pair = std::make_pair(std::min(var_id_x, var_id_y),
                                              std::max(var_id_x, var_id_y));
            auto it = std::find(indices.begin(), indices.end(), var_id_pair);
            if (it != indices.end()) {
                int current_index = std::distance(indices.begin(), it);
