...
double Term::get_derivative(int wrt, const std::vector<double> values) {
    ...
    switch (_type) {
        ...
        case TermType::quadratic:
            ...
            break;
        case TermType::cubic:
            return 3.0 * _coefficient * values[0] * values[0];
            break;
        case TermType::x_sq_y_sq:
            if (wrt == 0)
        ...
        case TermType::x_cube_y_cube:
            if (wrt == 0)
                return 3.0 * _coefficient * std::pow(values[0], 2) * std::pow(values[1], 3);
            else
                return 3.0 * _coefficient * std::pow(values[1], 2) * std::pow(values[0], 3);
            break;
        case TermType::x_abs_x:
...
double Term::get_second_derivative(int wrt, const std::vector<double> values) {
    ...
    switch (_type) {
        ...
        case TermType::quadratic:
            ...
            break;
        case TermType::cubic:
            return 6.0 * _coefficient * values[0];
            break;
        case TermType::x_sq_y_sq:
            if (wrt == 0)
        ...
        case TermType::x_cube_y_cube:
            if (wrt == 0)
                return 6.0 * _coefficient * values[0] * std::pow(values[1], 2);
            else
                return 6.0 * _coefficient * values[1] * std::pow(values[0], 2);
            break;
        case TermType::x_abs_x:
            if (values[0] > 0.0)
                return 2.0 * _coefficient;
...
double Term::get_mixed_derivative(const std::vector<double> values) {
    ...
    switch (_type) {
        ...
        case TermType::x_sq_y_sq:
            ...
            break;
        case TermType::x_cube_y_cube:
            return 9.0 * _coefficient * std::pow(values[0], 2) * std::pow(values[1], 2);
            break;
        case TermType::x_abs_x:
            return 0.0;
            break;
...
std::ostream &operator<<(std::ostream &os, const Term &term) {
    switch (term._type) {
        ...
        case TermType::cubic:
            if (term._coefficient != 1.0)
                os << "(" << std::to_string(term._coefficient) << ")"
                << " * pow(" << term._variables[0]->get_name() << ", 3)";
            else
                os << "pow(" << term._variables[0]->get_name() << ", 3)";
            break;
        case TermType::x_sq_y_sq:
            if (term._coefficient != 1.0)
                os << "(" << std::to_string(term._coefficient) << ")"
        ...
        case TermType::x_cube_y_cube:
            if (term._coefficient != 1.0)
                os << "(" << std::to_string(term._coefficient) << ")"
                << " * pow(" << term._variables[0]->get_name() << ", 3)"
                << " * pow(" << term._variables[1]->get_name() << ", 3)";
            else
                os << " * pow(" << term._variables[0]->get_name() << ", 3)"
                << " * pow(" << term._variables[1]->get_name() << ", 3)";
            break;
