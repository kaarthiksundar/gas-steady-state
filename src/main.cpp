...
int main(int argc, char *argv[]) {
    ...
    opt.add_option("c", "num_instances",
                   "number of instances of the case apart from the base case",
                   "0");
    ...
    std::cout << "*** Reading the input parameters and network data for the "
                 "base case "
              << std::endl;
    InputParams ip_base_case =
        build_input_params(opt["p"], base_case, opt["f"]);
    ConversionFactors cf_base_case(ip_base_case);
    Data data_base_case(opt["p"], base_case, opt["f"],
                        ip_base_case.get_units());
    ScalingFactors sf_base_case =
        build_scaling_factors(data_base_case.get_slack_pmin(), cf_base_case);
    ...
    std::cout
        << "*** Parameters required for formulating base case steady state "
           "model created "
        << std::endl;
    ...
    SteadyStateSolution sss_base_case(data_base_case, ssd_base_case,
                                      ssp_base_case, ip_base_case);
    ...
    sss_base_case.write_output(data_base_case, ip_base_case, opt["o"],
                               base_case, opt["f"]);
    ...
    for (auto case_name : other_cases) {
        std::cout << "*** Reading the input parameters and network data for "
                     "the case:  "
                  << case_name << std::endl;
        ...
        std::cout << "*** Parameters required for formulating base case steady "
                     "state model created "
                  << std::endl;
