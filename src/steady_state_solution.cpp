...
void SteadyStateSolution::write_output(const Data & data, std::string path) {
    /* write the csv files */
    CSVWriter pipe_flow_in(","), pipe_flow_out(","), pipe_pressure_in(","), pipe_pressure_out(",");
    ...
    gnode_supply_flows.writeToFile(path + OutputFilenames::gnode_supply_flows);
    gnode_demand_flows.writeToFile(path + OutputFilenames::gnode_demand_flows);
    
    CSVWriter comp_flow_in(","), comp_flow_out(",");
    CSVWriter comp_ratio(","), comp_power(",");
    CSVWriter comp_pressure_in(","), comp_pressure_out(","), comp_discharge_pressure(",");
    ...
    for (auto i=0; i<data.get_compressors().size(); ++i) {
        ...
        comp_discharge_pressure << std::to_string(_comp_discharge_pressure[i]);
    ...
    comp_pressure_out.writeToFile(path + OutputFilenames::comp_pressure_out);
    comp_discharge_pressure.writeToFile(path + OutputFilenames::comp_discharge_pressure);
...
void SteadyStateSolution::write_output(const Data & data, const InputParams & ip, 
                                        std::string data_path, std::string case_name, std::string data_format) {
    std::string path; 
    std::string filename;
    /* adjusting the path and file according for the data format provided */
    if (data_format == "csv") { path = data_path + case_name + "/"; filename = ""; }
    else { path = data_path + case_name; filename = path + "_output.json"; }
    if (data_format == "csv") {
        /* write the csv files */
        CSVWriter pipe_flow_in(","), pipe_flow_out(","), pipe_pressure_in(","), pipe_pressure_out(",");
        pipe_flow_in.enableAutoNewRow(data.get_pipes().size());
        ...
        std::map<std::string, double> pipe_flow_in, pipe_flow_out, pipe_pressure_in, pipe_pressure_out;
        for (auto i=0; i<data.get_pipes().size(); ++i) {
        ...
        std::map<std::string, double> comp_flow_in, comp_flow_out, comp_ratio, comp_power;
        std::map<std::string, double> comp_inlet_pressure, comp_discharge_pressure;
        for (auto i=0; i<data.get_compressors().size(); ++i) {
            std::string id = std::to_string(data.get_compressors()[i]->get_id());
